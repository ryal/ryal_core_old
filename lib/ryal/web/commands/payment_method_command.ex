defmodule Ryal.PaymentMethodCommand do
  ...
  def create(changeset) do
    with {:ok, payment_method} <- Core.repo.insert(changeset),
         ...
         {:ok, _default_payment_gateway} <-
            create_with_default_payment_gateway(payment_method, payment_gateways),
         {:ok, [_]} <-
            create_with_fallback_payment_gateways(payment_method, payment_gateways),
  ...
  defp create_with_default_payment_gateway(payment_method, payment_gateways) do
    payment_gateways
    |> Enum.find(&default_payment_gateway?/1)
    |> create_payment_method_gateway(payment_method)
  end

  defp create_with_fallback_payment_gateways(payment_method, payment_gateways) do
    payment_gateways
    |> Enum.reject(&default_payment_gateway?/1)
    |> Enum.each(fn(payment_gateway) ->
      spawn_monitor fn ->
        create_payment_method_gateway(payment_gateway, payment_method)
      end
    end)
  end

  defp create_payment_method_gateway(payment_gateway, payment_method) do
